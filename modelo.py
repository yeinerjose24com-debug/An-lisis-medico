...
def validar_sintomas(sintomas):
    ...
    age = convertir_a_float(sintomas.get('age'))
    if age == "error" or (age is not None and not (0 < age < 120)):
        errores.append("La edad debe ser un valor numérico entre 1 y 119.")

    temp = convertir_a_float(sintomas.get('body_temperature'))
    if temp == "error" or (temp is not None and not (34 < temp < 45)):
        errores.append("La temperatura corporal debe estar entre 34°C y 45°C.")

    hemoglobin = convertir_a_float(sintomas.get('hemoglobin'))
    if hemoglobin == "error" or (hemoglobin is not None and not (5 < hemoglobin < 25)):
        errores.append("El valor de hemoglobina debe estar entre 5 y 25 g/dL.")
...
def cargar_modelo():
    ...
    if os.path.exists(MODEL_PATH):
        ...
        if "model_logistica" in modelo_data and "model_mlp" in modelo_data:
            ...
            print(" Modelo antiguo detectado. Forzando re-entrenamiento para actualizarlo...")
            entrenar_modelo()
    else:
        print("No existe modelo entrenado. Entrenando nuevo modelo...")
        entrenar_modelo()
...
def entrenar_modelo():
    ...
    if target_col_name not in data.columns:
        print(" No se encontró la columna 'disease'. Buscando columnas similares...")
...
# INICIALIZACIÓN
# ===============================
# Nota: no cargamos el modelo automáticamente durante la importación
# para evitar impresiones o ejecuciones innecesarias. La aplicación
# (app.py) llama a cargar_modelo() en su bloque __main__.

# ===============================
# TEST LOCAL
# ===============================
if __name__ == "__main__":
    # al ejecutar directamente podemos cargar el modelo para pruebas locales
    cargar_modelo()
    print("\n Test de ejemplo:")
    ...
    print(f"Precisión (Logística): {resultados['logistica']['exactitud']*100:.2f}%")
    print(f"Precisión (Red Neuronal): {resultados['red_neuronal']['exactitud']*100:.2f}%")
